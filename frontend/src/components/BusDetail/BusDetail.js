...
const BusDetail = (props) => {
    ...
    const buttonClicked= (index) => {
        ...
        if(bookArray.indexOf(index)===-1) {
            ...
            temp.splice(ind,1);
            document.getElementById(`seat${index}`).style.backgroundColor = 'lightgreen';
        }
        setBookArray(temp);
        temp = bookArray.map((seat, index) => {
            return (<Button variant="contained" style={{margin: '10px auto', border: '2px solid blue', backgroundColor: 'yellow'}} key={index}>{seat}</Button>);
        });
        setSeatArr(
    ...
    const sendTickets = async() => {
        ...
        setLoadingPass(true);
        const [isHttpError, responseData] = await httpReq(`/booking/bookTicket`,
        'POST',
        {
        ...
        if(!isHttpError) {
            setTickets(responseData.tickets.map((ticketId, index)=> {
                return (
                    <Container key={index} maxWidth="sm" style={{textAlign: 'center'}}>
    ...
    useEffect(()=> {
        setPassengerArray(passObj.map((object, index)=> {
            return <Container maxWidth="sm" index={index} key={index} style={{textAlign: 'center', display: 'flex', marginTop: '5px'}}>
                <Button variant="contained" style={{margin:'0px auto', backgroundColor: 'lightgreen', padding: '0px', height: '55px'}}>{bookArray[index]}</Button>
                <TextField  style={{marginRight: '10px'}} label="Passenger Name" index={index}
                value={object.name}
                onChange={(e)=> {changeNameHandler(e, index)}}
                helperText="Add Passenger Name"
                variant="filled"/>
                <TextField type="number" label="Passenger Age" index={index}
                value={object.age}
                onChange={(e)=> {changeAgeHandler(e, index)}}
        ...
        for(let seatRow=0;seatRow<10;seatRow++) {
            totalSeats.push(
                <Container maxWidth="sm" key={seatRow} style={{display: 'flex'}}>
                    <Button id={`seat${seatRow*4 + 1}`} onClick={()=> {buttonClicked(seatRow*4 +1)}} variant="contained" style={{margin:'5px auto', backgroundColor: 'lightgreen'}}>
                        {seatRow*4 + 1}
                    </Button>
                    <Button id={`seat${seatRow*4 + 2}`} onClick={()=> {buttonClicked(seatRow*4 +2)}} variant="contained" style={{margin:'5px auto', backgroundColor: 'lightgreen'}}>
                        {seatRow*4 + 2}
                    </Button>
                    <Button id={`seat${seatRow*4 + 3}`} onClick={()=> {buttonClicked(seatRow*4 +3)}} variant="contained" style={{margin:'5px auto', backgroundColor: 'lightgreen'}}>
                        {seatRow*4 + 3}
                    </Button>
                    <Button id={`seat${seatRow*4 + 4}`} onClick={()=> {buttonClicked(seatRow*4 +4)}} variant="contained" style={{margin:'5px auto', backgroundColor: 'lightgreen'}}>
                        {seatRow*4 + 4}
                    </Button>
                </Container>
            );
    ...
    <div className="bus-detail">
        ...
        <Container id="seats-to-book" style={{textAlign: 'center', margin: '35px 0px'}}>
            <Typography variant="h4" style={{textAlign: 'center'}} gutterBottom>
                Seats to Book:
            </Typography>
        </Container>
        {bookArray && bookArray.length>0 && passObj &&(
            <div>
                ...
                <Button id="get-name-btn" onClick={getNames} variant="contained" style={{textAlign: 'center', margin:'40px auto', display: 'block',backgroundColor: 'lightgreen'}}>Book Selected Tickets</Button>
                <div id="pass-details">
                    <Container id="email-text" maxWidth="sm" style={{textAlign: 'center', display: 'none'}}>
                        <TextField label="Passenger Email"
                            value={email}
                ...
                <Button id="send-ticket-btn" onClick={sendTickets} variant="contained" style={{textAlign: 'center', margin:'40px auto', display: 'none',backgroundColor: 'lightgreen'}}>Confirm Book</Button>
                {loadingPass && (
                ...
                {tickets && (
                    <Container maxWidth="sm" style={{textAlign: 'center'}} >
                        <Typography variant="h4" style={{textAlign: 'center'}} gutterBottom>
                            Seats Booked and sent to your email successfully!<br />
                        </Typography>
                    </Container>
                )}
                {(tickets)}
